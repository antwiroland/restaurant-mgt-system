...
export default function UsersPage() {
  ...
  if (!session) return <main className="shell"><section className="panel"><p className="kicker">Users</p><h1 className="text-2xl font-semibold">Staff access required</h1><Link href="/login" className="mt-4 inline-flex rounded-full bg-[#132018] px-4 py-2 text-white">Open Login</Link></section></main>;
    ...
    <main className="shell grid gap-4">
      <section className="panel">
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-3">
          <input className="rounded-xl border border-[#cfe0c8] p-2 text-sm" placeholder="Search name, phone, or email" value={query} onChange={(e) => setQuery(e.target.value)} />
          <select className="rounded-xl border border-[#cfe0c8] p-2 text-sm" value={roleFilter} onChange={(e) => setRoleFilter(e.target.value as StaffRole | "ALL")}>
      ...
      <section className="panel">
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-3">
          <input className="rounded-xl border border-[#cfe0c8] p-2 text-sm" placeholder="Name" value={newUser.name} onChange={(e) => setNewUser((s) => ({ ...s, name: e.target.value }))} />
          <input className="rounded-xl border border-[#cfe0c8] p-2 text-sm" placeholder="Phone" value={newUser.phone} onChange={(e) => setNewUser((s) => ({ ...s, phone: e.target.value }))} />
          <input className="rounded-xl border border-[#cfe0c8] p-2 text-sm" placeholder="Email (optional)" value={newUser.email} onChange={(e) => setNewUser((s) => ({ ...s, email: e.target.value }))} />
          <input className="rounded-xl border border-[#cfe0c8] p-2 text-sm" placeholder="Password (min 6 chars)" type="password" value={newUser.password} onChange={(e) => setNewUser((s) => ({ ...s, password: e.target.value }))} />
          <select className="rounded-xl border border-[#cfe0c8] p-2 text-sm" value={newUser.role} onChange={(e) => setNewUser((s) => ({ ...s, role: e.target.value as StaffRole }))}>
            {STAFF_ROLES.map((role) => <option key={role} value={role}>{role}</option>)}
          </select>
          <select className="rounded-xl border border-[#cfe0c8] p-2 text-sm" value={newUser.branchId} onChange={(e) => setNewUser((s) => ({ ...s, branchId: e.target.value }))}>
            <option value="">No branch</option>
            {branchOptions.map((branch) => <option key={branch.id} value={branch.id}>{branch.label}</option>)}
          </select>
          <input className="rounded-xl border border-[#cfe0c8] p-2 text-sm" placeholder="Optional 4-digit PIN" maxLength={4} value={newUser.pin} onChange={(e) => setNewUser((s) => ({ ...s, pin: e.target.value }))} />
          <button className="rounded-full border border-[#132018] px-3 py-2 text-xs md:col-span-2" onClick={() => void createStaffUser()}>Create User</button>
        </div>
      </section>
      ...
      <section className="panel">
        <ul className="grid gap-3">
          {filteredUsers.map((user) => (
            <li key={user.id} className="rounded-xl border border-[#cfe0c8] p-3">
              <p className="font-semibold">{user.name} ({user.phone})</p>
              <p className="text-xs text-[#35523d]">Current: {user.role}{user.branchName ? ` - ${user.branchName}` : ""}</p>

              <div className="mt-2 flex flex-wrap items-center gap-2">
                <select className="rounded-xl border border-[#cfe0c8] p-2 text-sm" defaultValue={user.role} onChange={(e) => void updateRole(user, e.target.value as StaffRole, user.branchId)}>
                  {STAFF_ROLES.map((role) => <option key={role} value={role}>{role}</option>)}
                </select>
                <select className="rounded-xl border border-[#cfe0c8] p-2 text-sm" defaultValue={user.branchId ?? ""} onChange={(e) => void updateRole(user, user.role, e.target.value || undefined)}>
                  <option value="">No branch</option>
                  {branchOptions.map((branch) => <option key={branch.id} value={branch.id}>{branch.label}</option>)}
                </select>
              </div>
              ...
              <div className="mt-2 flex items-center gap-2">
                <input
                  className="rounded-xl border border-[#cfe0c8] p-2 text-sm"
                  placeholder="Set 4-digit PIN"
                  value={pinByUserId[user.id] ?? ""}
                  onChange={(e) => setPinByUserId((current) => ({ ...current, [user.id]: e.target.value }))}
                  maxLength={4}
                />
                <button className="rounded-full border border-[#132018] px-3 py-1 text-xs" onClick={() => void applyPin(user)}>Set PIN</button>
              </div>
            </li>
          ))}
          {filteredUsers.length === 0 ? <li className="rounded-xl border border-dashed border-[#cfe0c8] p-3 text-sm text-[#35523d]">No users match the current filters.</li> : null}
        </ul>
      </section>
    </main>
  );
