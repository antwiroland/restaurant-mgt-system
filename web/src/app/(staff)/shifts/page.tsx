  ...
  return (
    <main className="shell grid gap-4 md:grid-cols-[1fr,1.4fr]">
      <section className="panel">
        ...
        <p className="mt-2 text-sm text-ink-soft">Start cashier shift and capture opening drawer amount.</p>
        ...
        <div className="mt-4 grid gap-3">
          <label className="grid gap-2 text-sm text-ink-soft">
            ...
            />
            {openFieldError ? <span className="text-xs text-danger-on">{openFieldError}</span> : null}
          </label>
      ...
      <section className="panel">
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-2">
          <input className="input text-sm" placeholder="Search cashier or branch" value={query} onChange={(e) => setQuery(e.target.value)} />
        ...
        <div className="mt-4 grid gap-3">
          {filteredShifts.length === 0 ? (
            <p className="rounded-xl border border-dashed border-line p-4 text-sm text-ink-soft">No active shifts.</p>
          ) : (
            filteredShifts.map((shift) => (
              <article key={shift.id} className="rounded-xl border border-line p-4">
                <p className="font-semibold">{shift.cashierName}</p>
                <p className="text-sm text-ink-soft">{shift.branchName ?? "No branch"}</p>
                <p className="mt-2 text-sm text-ink-soft">Opened {new Date(shift.openedAt).toLocaleString()}</p>
                <p className="mt-1 text-sm text-ink-soft">Opening Cash: GHS {Number.parseFloat(shift.openingCash).toFixed(2)}</p>
                ...
                <div className="mt-3 grid gap-2 md:grid-cols-2">
                  <div className="grid gap-1">
                    ...
                    />
                    {closeFieldErrors[shift.id] ? <span className="text-xs text-danger-on">{closeFieldErrors[shift.id]}</span> : null}
                  </div>
                  <input
                    className="input text-sm"
                    type="text"
                ...
                <button
                  type="button"
                  className="mt-3 btn btn-secondary disabled:opacity-60"
