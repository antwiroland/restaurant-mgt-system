...
export default function TableBillPage() {
  ...
  if (!session) return <main className="shell"><section className="panel"><p className="kicker">Table Bill</p><h1 className="text-2xl font-semibold">Staff access required</h1><Link href="/login" className="mt-4 inline-flex rounded-full bg-[#132018] px-4 py-2 text-white">Open Login</Link></section></main>;
  ...
  return (
    <main className="shell">
      <section className="panel">
        ...
        {bill ? (
          <div className="mt-4 rounded-xl border border-[#cfe0c8] p-4 text-sm text-[#35523d]">
        ...
        <div className="mt-4 flex gap-2">
          <button className="rounded-full border border-[#132018] px-4 py-2 text-sm text-[#132018]" onClick={() => void closeTable()}>Close Table</button>
          <button className="rounded-full border border-[#991b1b] px-4 py-2 text-sm text-[#991b1b]" onClick={() => void reverseTable()}>Reverse Table</button>
          <Link className="rounded-full border border-[#132018] px-4 py-2 text-sm text-[#132018]" href="/tables">Back</Link>
        </div>
      </section>
    </main>
  );
