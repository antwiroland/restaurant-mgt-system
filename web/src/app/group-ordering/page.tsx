...
export default function GroupOrderingPage() {
  ...
  if (!session) return <main className="shell"><section className="panel"><p className="kicker">Group Ordering</p><h1 className="text-2xl font-semibold">Staff access required</h1><Link href="/login" className="mt-4 inline-flex rounded-full bg-[#132018] px-4 py-2 text-white">Open Login</Link></section></main>;
  ...
  return (
    <main className="shell grid gap-4">
      <section className="panel">
        ...
        <p className="mt-2 text-sm text-[#35523d]">Create or join a session, add participant items, and finalize to one order.</p>
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <article className="panel">
          ...
          <form className="mt-3 grid gap-2" onSubmit={(event) => void onCreateSession(event)}>
            <input className="rounded-xl border border-[#cfe0c8] p-2" placeholder="Host display name" value={createName} onChange={(e) => setCreateName(e.target.value)} />
        ...
        <article className="panel">
          ...
          <form className="mt-3 grid gap-2" onSubmit={(event) => void onJoinSession(event)}>
            <input className="rounded-xl border border-[#cfe0c8] p-2" placeholder="Session code" value={joinCode} onChange={(e) => setJoinCode(e.target.value.toUpperCase())} required />
            <input className="rounded-xl border border-[#cfe0c8] p-2" placeholder="Participant display name" value={joinName} onChange={(e) => setJoinName(e.target.value)} />
            <button className="rounded-full bg-[#132018] px-4 py-2 text-white">Join</button>
          </form>
        </article>
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <article className="panel">
          ...
          <form className="mt-3 grid gap-2" onSubmit={(event) => void onAddItem(event)}>
            <input className="rounded-xl border border-[#cfe0c8] p-2" placeholder="Active session code" value={sessionCode} onChange={(e) => setSessionCode(e.target.value.toUpperCase())} required />
            <input className="rounded-xl border border-[#cfe0c8] p-2" placeholder="Participant ID" value={participantId} onChange={(e) => setParticipantId(e.target.value)} required />
            <select className="rounded-xl border border-[#cfe0c8] p-2" value={itemMenuId} onChange={(e) => setItemMenuId(e.target.value)}>
              {menuItems.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
            </select>
            <input className="rounded-xl border border-[#cfe0c8] p-2" type="number" min={1} placeholder="Quantity" value={itemQty} onChange={(e) => setItemQty(e.target.value)} required />
            <input className="rounded-xl border border-[#cfe0c8] p-2" placeholder="Notes" value={itemNotes} onChange={(e) => setItemNotes(e.target.value)} />
            <button className="rounded-full border border-[#132018] px-4 py-2 text-[#132018]">Add Item</button>
          </form>
          {selectedItem ? <p className="mt-2 text-xs text-[#35523d]">Selected: {selectedItem.name} ({selectedItem.price})</p> : null}
          <button className="mt-3 rounded-full border border-[#132018] px-3 py-1 text-xs" onClick={() => void refreshView()}>Refresh Session</button>
        </article>
        ...
        <article className="panel">
          ...
          <form className="mt-3 grid gap-2" onSubmit={(event) => void onFinalize(event)}>
            <select className="rounded-xl border border-[#cfe0c8] p-2" value={finalizeType} onChange={(e) => setFinalizeType(e.target.value as "DINE_IN" | "PICKUP" | "DELIVERY")}>
              <option value="DINE_IN">DINE_IN</option>
              <option value="PICKUP">PICKUP</option>
              <option value="DELIVERY">DELIVERY</option>
            </select>
            {finalizeType === "DINE_IN" ? (
              <select className="rounded-xl border border-[#cfe0c8] p-2" value={finalizeTableId} onChange={(e) => setFinalizeTableId(e.target.value)}>
                {tables.map((table) => <option key={table.id} value={table.id}>Table {table.number}</option>)}
              </select>
            ) : null}
            {finalizeType === "DELIVERY" ? (
              <input className="rounded-xl border border-[#cfe0c8] p-2" placeholder="Delivery address" value={finalizeAddress} onChange={(e) => setFinalizeAddress(e.target.value)} required />
            ) : null}
            <input className="rounded-xl border border-[#cfe0c8] p-2" placeholder="Finalize notes" value={finalizeNotes} onChange={(e) => setFinalizeNotes(e.target.value)} />
            <button className="rounded-full bg-[#132018] px-4 py-2 text-white">Finalize Group</button>
          </form>
        </article>
      </section>
      ...
      <section className="panel">
        ...
        {!groupView ? (
          <p className="mt-3 text-sm text-[#35523d]">No session loaded.</p>
        ) : (
          <div className="mt-3 grid gap-3">
            ...
            {groupView.participants.map((participant) => (
              <article key={participant.participantId} className="rounded-xl border border-[#cfe0c8] p-3">
