...
public class JwtService {
    ...
    public String generateOverrideToken(UUID userId, Role role, OverrideActionType actionType) {
        return generateToken(userId, role, TokenType.OVERRIDE, actionType,
    ...
    public String generateRefreshTokenWithTtl(UUID userId, Role role, Duration ttl) {
        return generateToken(userId, role, TokenType.REFRESH, null, ttl);
    }

    public String generateAccessTokenWithTtl(UUID userId, Role role, Duration ttl) {
        return generateToken(userId, role, TokenType.ACCESS, null, ttl);
    }

    public String generateOverrideTokenWithTtl(UUID userId, Role role, OverrideActionType actionType, Duration ttl) {
        return generateToken(userId, role, TokenType.OVERRIDE, actionType, ttl);
    }

    private String generateToken(UUID userId, Role role, TokenType tokenType, OverrideActionType actionType, Duration ttl) {
